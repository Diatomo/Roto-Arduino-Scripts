 ...
 *
 *	FxN :: addLink
 *		@address :: address of the 7-seg <check DIP switch>
 *		@groupID :: group of ID of a section inside the linked list.
 *
 */
Segment DigitLink::addLink(uint8_t address, uint8_t groupID, Segment currNode, Segment prevNode){
...
void DigitGroup::update(){
	...
	delay(500);
	mySerial.write(head->address);
	mySerial.write(0x43); 
	mySerial.write(head->number + 0x80);
	uint8_t checksum = ((head->address + 0x43 + head->number + 0x80) % 64) + 0xC0;
...
//weird private mapping function
uint8_t _digits_mapToSegs(uint8_t i){
	uint8_t const  segMap[10] =
